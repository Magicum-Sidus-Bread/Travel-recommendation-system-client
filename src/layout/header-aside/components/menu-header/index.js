...
export default {
  ...
  methods: {
    // 动态菜单赋值
    async getHeaderMenu () {
      const menuHeader__ = await getHeadMenu()
      this.$store.commit('d2admin/menu/headerSet', menuHeader__)
      // console.log(menuHeader__)
      // 初始化菜单搜索功能
      this.$store.commit('d2admin/search/init', menuHeader__)
    },
    scroll (direction) {
      if (direction === 'left') {
        // 向右滚动
