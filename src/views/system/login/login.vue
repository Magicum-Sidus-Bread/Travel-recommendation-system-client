        ...
        <div
          ...
          <div class="page-login--form">
            ...
            <p
              ...
              <span><d2-icon name="question-circle"/> <el-link
                type="primary"
                :underline="false"
                @click="toLink('/forget')"
              >找回密码
...
import { Login, LoginVerificationCheck } from '@/api/modules/user/sys.auth'
        ...
        if (valid) {
          ...
          Login({
            ...
            if (res.Success) {
              ...
              util.cookies.set('email', res.Data.Email)
              util.cookies.set('avatar', res.Data.Avatar)
              // 设置用户名称
              this.$store.dispatch('d2admin/user/set', { name: res.Data.Name }, { root: true })
              // 用户登录后从持久化数据加载一系列的设置
              ...
              this.$message.info(`登录失败 ${res.Message}`)
              this.getCode()
            }
          }).catch(err => {
            console.log(err)
