...
import { Register, LoginVerificationCheck } from '@/api/modules/user/sys.auth'
export default {
  mixins: [
    localeMixin
  ],
  ...
  methods: {
    ...
    handleUserBtnClick (user) {
      ...
      this.formLogin.repeat_password = user.password
      this.submit()
    },
    /**
     * @description 提交表单
    ...
    submit () {
      this.$refs.loginForm.validate(async (valid) => {
        if (valid) {
          ...
          if (this.formLogin.password !== this.formLogin.repeat_password) {
            this.$message.error('请检查两次密码输入')
            console.log(this.is_code_true)
            this.getCode()
            return
          }
          // 调用注册接口
